...
class ImageEditor:
    def apply_binarization(self, gray, method='standard'):
        if method == 'standard' or method == 'raw':
    ...
    def crop_image(self, image, margin=10, image_raw=None):
        ...
        for contour in contours:
            ...
            cv2.imwrite('BIN.png', masked_image[y:y+h, x:x+w])
            coordinates.append((x, y, w, h))

        if image_raw is not None:
            for contour in contours:
                cropped_raw_images.append(image_raw[y:y+h, x:x+w])
                cv2.imwrite('RAW.png', image_raw[y:y+h, x:x+w])
...
if __name__ == "__main__":
    ...
    camera_handler = CameraHandler('CameraParams.npz')
    image_editor = ImageEditor()
    feature_matcher = FeatureMatcher()
    feature_processor = FeatureProcessor(camera_handler, image_editor, feature_matcher)

    feature_processor.clear_directory()

    reference_names = [f'ref{i}_{j}' for i in range(1, 6) for j in range(1, 2)]
    logging.info(f"Reference names: {reference_names}")
    
    binarization_types = ['raw', 'standard', 'adaptive', 'otsu'] #, 'canny']
    
    
    # reference_names = ['ref3_1']
    # binarization_types = ['raw', 'standard']

    for reference_name in reference_names:
        for binarization_type in binarization_types:
            feature_processor.run_feature_matching(reference_name, binarization_type)
